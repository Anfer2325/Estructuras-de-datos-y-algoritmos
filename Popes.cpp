...
int main(){
	DCola<int> c;
	int numAnyosMax, numPapas, anyo=0, primero=99, ultimo=99, papasMax=0; 
	cin >> numAnyosMax; //Leo el periodo de años en el que vamos a buscar
	cin >> numPapas; // Leo cuanto papas voy a leer
	while(numPapas!=0){
		cin >> anyo;
		numPapas--;
		while(numPapas!=0 && (c.esVacia() || anyo-c.primero()<numAnyosMax)){ // voy metiendo en la cola mientras esten dentro del periodo
			c.ponDetras(anyo);
			cin >> anyo;
			numPapas--;
		}
		if(c.numElems()>papasMax){ // compruebo aver si hay más papas en este periodo
			papasMax = c.numElems();
			primero = c.primero();
			ultimo = c.ultimo();
		}

		while(!c.esVacia() && anyo-c.primero()>=numAnyosMax){ // Le hago un hueco al nuevo papa para que queda en el nuevo periodo
			c.quitaPrim();
		}
		
		c.ponDetras(anyo); // Meto al nuevo papa en el nuevo periodo
		
	}
	cout << papasMax << " " << primero << " " << ultimo << endl;
	return 0;
}