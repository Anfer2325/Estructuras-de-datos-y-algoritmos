...
void rails(){
	string lee,cadena;
	int numVagones,i=0, tren;
	bool ok = true;
	Cola<string> ins;
	Cola<int> c;
	Pila<int> digitos, pila;
	/*do{
		getline(cin,lee);
		ins.ponDetras(lee);
	}while(!cin.eof());*/
	...
	while(!ins.esVacia()){
		...
		while(numVagones!=0){
			...
			cadena = ins.primero();
			ins.quitaPrim();
			tren = atoi(cadena.c_str());
			if(tren!=0){
				while(tren!=0){
					digitos.apila(tren%10);
					tren=tren/10;
				}

				while(!digitos.esVacia()){
					if(digitos.cima()==c.primero()){
						c.quitaPrim();
					}else{
						pila.apila(digitos.cima());
					}
					digitos.desapila();
				}

				while(!pila.esVacia()){
					if(pila.cima()==c.primero()){
						c.quitaPrim();
					}else{
						ok=false;
					}
