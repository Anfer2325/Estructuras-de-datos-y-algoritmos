...
void solucion(Arbus <string, Lista<string>> p, Arbus <string, Lista<string>> a){
	Arbus<string, int> proyectos;
	Arbus <string, Lista<string>>::Iterador it = p.principio();
	while(it!=p.final()){
		string clave = it.clave();
		Lista<string> valor = it.valor();
		proyectos.inserta(clave, valor.numElems());
		it.avanza();
	}
	Arbus<string, int>::Iterador it2 = proyectos.principio();
	while(it2!=proyectos.final()){
		cout << it2.clave() << " " << it2.valor() << endl;
		it2.avanza();
	}
...
int main(){
	Arbus <string, Lista<string>> p; // proyecto con sus alumnos
	Arbus <string, Lista<string>> a; // alumnos con sus proyectos participados
	string linea, proyecto; // linea que lees, nombre del proyecto
	...
	while(linea!="0"){
		...
		while(linea!="1"){
			...
			if(isupper(linea[0])){ // Tengo un nuevo proyecto
				proyecto = linea;
			}
			
			else{  // Más alumnos al proyecto
				if(a.esta(linea)){ //Buscamos si esta repetido el alumno
					Lista<string> proyectos = a.consulta(linea); //proyectos del alumno
					Lista<string>::Iterador it = proyectos.principio();
					while(it!=proyectos.final()){ //Buscamos en que proyecto tenemos que quitar al alumno
						string pr = it.elem();
						if(p.esta(pr)){
							Lista<string> alumnos = p.consulta(pr); // Alumnos del proyecto
							Lista<string>::Iterador itAl = alumnos.principio();
							while(itAl!=alumnos.final()){ //Quitamos al alumno del proyecto en el que este
								if(it.elem()==linea){
									itAl = alumnos.borra(itAl);
								}
								itAl.avanza();
							}
							p.inserta(pr, alumnos);
						}
						it.avanza();
					}
				}else{
					Lista<string> lista1;
					lista1.ponDr(proyecto);
					a.inserta(linea, lista1);
					if(p.esta(proyecto)){
						Lista<string>alumnos = p.consulta(proyecto);
						alumnos.ponDr(linea);
						p.inserta(proyecto, alumnos);
					}else{
						Lista<string> alumnos;
						alumnos.ponDr(linea);
						p.inserta(proyecto, alumnos);
					}
				}
