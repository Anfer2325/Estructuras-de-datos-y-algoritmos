...
int main(){
	Arbus <int, int> a;
	char c;
	int numBloques,numTelefonos, ceros=6, cerosAux, num=0, d;
	cin >> numBloques;
	ignoraEspacios(cin);
	while(numBloques!=0){
		cin >> numTelefonos;
		...
		while(numTelefonos!=0){
			...
			if(c=='A'||c=='B'||c=='C'){
				...
				if(a.esta(num)){
					a.inserta(num,a.consulta(num)+1);
				}else{
					a.inserta(num,1);
				}
				num=0; ceros=7; d=0, cerosAux = 0, numTelefonos--;
			}else if(c >= '0' && c<='9'){
				d=c-'0';
			}else{
				cerosAux = 0; ceros++; d=0;
			}
			ceros--;
			while(cerosAux!=0){
				d = d*10;
	...
	Arbus<int, int>::Iterador it = a.principio();
	while (it != a.final()) {
		if(it.valor()>1){
			int imp1 = it.clave() / 10000;
			int imp2 = it.clave() % 10000;
			cout << imp1 << "-" << imp2 << " " << it.valor() << endl;
		}
		it.avanza();
	}
