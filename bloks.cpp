...
int buscaBloque(Lista<Pila<int>> lista);
Pila<int> subpila(Pila<int> p, int num);
Pila<int> cogeDeLaPila(Pila<int> pila, int num);
void blocks();
Lista<Pila<int>> moveOnto(Lista<Pila<int>> lista, int a, int b);
Lista<Pila<int>> moveOver(Lista<Pila<int>> lista, int a, int b);
Lista<Pila<int>> pileOnto(Lista<Pila<int>> lista, int a, int b);
Lista<Pila<int>> pileOver(Lista<Pila<int>> lista, int a, int b);
void resultado(Lista<Pila<int>> lista);
...
int buscaBloque(Lista<Pila<int>> lista, int numero){
	Lista<Pila<int>>::Iterador it = lista.principio();
	int i=0;
	bool encontrado = false;
	while(it!=lista.final()&&!encontrado){
		Pila<int> aux;
		aux = it.elem();
...
void blocks(){
	bool quit = false;
	Cola<string> instrucciones;
	Lista<Pila<int>> blocks;
	string ins;
	int numBlocks=0, i=0;
	cin>>numBlocks;
	while(numBlocks!=0){
	...
	/*do{
		getline(cin, ins);
		instrucciones.ponDetras(ins);
	}while(!cin.eof());*/
	
	instrucciones.ponDetras("move 9 onto 1");
	instrucciones.ponDetras("move 8 over 1");
	instrucciones.ponDetras("move 7 over 1");
	instrucciones.ponDetras("move 6 over 1");
	instrucciones.ponDetras("pile 8 over 6");
	instrucciones.ponDetras("pile 8 oover 5");
	instrucciones.ponDetras("move 2 oover 1");
	instrucciones.ponDetras("move 4 over 9");
	instrucciones.ponDetras("quit");

	while(!instrucciones.esVacia()&&!quit){
		ins = instrucciones.primero();
		if(ins[0]=='m'&&ins[1]=='o'&&ins[2]=='v'&&ins[3]=='e'){
			int a = ins[5] - '0';
			int b = ins[12] - '0';
			if(ins[7]=='o'&&ins[8]=='n'&&ins[9]=='t'&&ins[10]=='o'){
				blocks = moveOnto(blocks,a,b);
			}else if(ins[7]=='o'&&ins[8]=='v'&&ins[9]=='e'&&ins[10]=='r'){
				blocks = moveOver(blocks,a,b);
			}
		}else if(ins[0]=='p'&&ins[1]=='i'&&ins[2]=='l'&&ins[3]=='e'){
			int a = ins[5] - '0';
			int b = ins[12] - '0';
			if(ins[7]=='o'&&ins[8]=='n'&&ins[9]=='t'&&ins[10]=='o'){
				blocks = pileOnto(blocks,a,b);
			}else if(ins[7]=='o'&&ins[8]=='v'&&ins[9]=='e'&&ins[10]=='r'){
				blocks = pileOver(blocks,a,b);
			}
		}else if(ins[0]=='q'&&ins[1]=='u'&&ins[2]=='i'&&ins[3]=='t'){
			quit = true;
		}
		instrucciones.quitaPrim();
	}
	resultado(blocks);
}
Lista<Pila<int>> moveOnto(Lista<Pila<int>> lista, int a, int b){
	Lista<Pila<int>>::Iterador it = lista.principio();
	int pos = buscaBloque(lista,a); //Busco a
	while(pos!=0){
		it.avanza();
		pos--;
	}
	Pila<int> paux=it.elem();
	paux = cogeDeLaPila(paux, a); // Lo cogo de la pila
	lista.insertar(paux, it);
	it = lista.borra(it);
	it = lista.principio();
	while(b!=0){
		it.avanza();
...
Lista<Pila<int>> moveOver(Lista<Pila<int>> lista, int a, int b){
	...
	lista.insertar(paux, it);
	it = lista.borra(it);
	pos = buscaBloque(lista,b); // Busco b
	it = lista.principio();
	while(pos!=0){
		it.avanza();
		pos--;
...
Lista<Pila<int>> pileOnto(Lista<Pila<int>> lista, int a, int b){
	Lista<Pila<int>>::Iterador it = lista.principio(); //Busco a
	int pos = buscaBloque(lista,a); //Busco a
	while(pos!=0){
		it.avanza();
		pos--;
	...
	Pila<int> paux=it.elem();
	paux = subpila(paux, a); // Cogo la pila que tiene a como base
	Cola<int> subp;
	while(!paux.esVacia()){
		subp.ponDetras(paux.cima());
		paux.desapila();
	}
	lista.insertar(paux, it);
	it = lista.borra(it);
	it = lista.principio();
	pos = b; // Muevo el iterador al bloque b
	it = lista.principio();
	while(pos!=0){
		it.avanza();
		pos--;
	...
	while(!subp.esVacia()){
		paux=it.elem();
		paux.apila(subp.primero()); //Apilo subp en la pila b
		subp.quitaPrim();
	}
	lista.insertar(paux, it);
	it = lista.borra(it);
...
Lista<Pila<int>> pileOver(Lista<Pila<int>> lista, int a, int b){
	...
	lista.insertar(paux, it);
	it = lista.borra(it);
	pos = buscaBloque(lista,b); // Muevo el iterador al bloque b
	it = lista.principio();
	while(pos!=0){
		it.avanza();
		pos--;
