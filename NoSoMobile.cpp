...
bool leeArbol (Arbin<unsigned int> a, Arbin<unsigned int> b){
	unsigned int n1, n2, n3, n4;
	bool balanceado=true, recursionParcial=true;
	cin >> n1;
	cin >> n2;
	cin >> n3;
	cin >> n4;
	if(n1*n2==n3*n4){
		//cout << n1 << " y " << n2 << " estan balanceados con " << n3 << " y " << n4 << endl;
		balanceado = true;
	}else{
		//cout << n1 << " y " << n2 << " no estan balanceados con " << n3 << " y " << n4 << endl;
		balanceado = false;
	}

	if(n1==0){
		//cout << "Recursion por el hijo izquierdo" << endl;
		a = Arbin<unsigned int>(Arbin<unsigned int>(),n1,Arbin<unsigned int>());
		recursionParcial = balanceado && leeArbol(a.hijoIz(),a.hijoDr());
	}else{
		//cout << "Hijo izquiero vacio" << endl;
		a = Arbin<unsigned int>(Arbin<unsigned int>(),n1,Arbin<unsigned int>());
		return balanceado;
	}
	if(n3==0){
		//cout << "Recursion por el hijo derecho" << endl;
		b = Arbin<unsigned int>(Arbin<unsigned int>(),n3,Arbin<unsigned int>());
		recursionParcial = balanceado && leeArbol(b.hijoIz(),b.hijoDr());
	}else{
		//cout << "Hijo derecho vacio" << endl;
		b = Arbin<unsigned int>(Arbin<unsigned int>(),n3,Arbin<unsigned int>());
		return balanceado;
	}
	return balanceado && recursionParcial;
}

int main(){
	Arbin<unsigned int> a = Arbin<unsigned int>(Arbin<unsigned int>(),0,Arbin<unsigned int>());
	unsigned int numMoviles;
	bool balanceado = true;
	cin >> numMoviles;
	while(numMoviles!=0){
		if(leeArbol(a.hijoIz(),a.hijoDr())){
			cout << "SI" << endl;
		}else{
			cout << "NO" << endl;
		}
