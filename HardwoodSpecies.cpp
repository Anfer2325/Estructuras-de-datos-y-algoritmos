...
int main(){
	...
	while(!cin.eof()){
		while(getline(cin,linea)){
			numArboles++;
			if(arboles.esta(linea)){
				float valorAntiguo = arboles.consulta(linea);
				//cout << linea << "   " << valorAntiguo+1 << endl;
				arboles.inserta(linea,valorAntiguo+1);
				
			}else{
				//cout << linea << "   " << 1 << endl;
				arboles.inserta(linea,1);
			}
			
			
		}
		
		Arbus<string,float>::Iterador it = arboles.principio();
		while(it!=arboles.final()){
			string arbol = it.clave();
			float porcentaje = it.valor() * 100 / numArboles;
			porcentaje=float(int(porcentaje*10000.0 + 0.5))/10000.0;
			cout << arbol << " " << porcentaje << endl;
			it.avanza();
			arboles.borra(arbol);
		}
