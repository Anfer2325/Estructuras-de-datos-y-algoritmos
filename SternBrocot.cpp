...
//Crea el arbol con solo los nodos necesarios
Arbin<Fraccion> crea (int num, int den, Fraccion f1, Fraccion f2){
	...
	if((double)num/den>fnueva.valor()){
		return Arbin<Fraccion>(Arbin<Fraccion>(), fnueva, crea(num, den, fnueva, f2));
	}else if((double)num/den<fnueva.valor()){
		return Arbin<Fraccion>(crea(num, den, f1, fnueva), fnueva, Arbin<Fraccion>());
	}else{
		return Arbin<Fraccion>(Arbin<Fraccion>(), fnueva, Arbin<Fraccion>());
	}
	
}

//Realiza la busqueda por el arbol y devuelve el string solucion
string soluciona(Arbin<Fraccion> arbol, int numerador, int denominador){
	if(arbol.esVacio()){
		return "";
	}else{
		double aBuscar = (double) numerador / denominador;
		double actual = (double)arbol.raiz().num / arbol.raiz().den;
		if(aBuscar > actual){
			return "R" + soluciona(arbol.hijoDr(), numerador, denominador);
		}else if(aBuscar < actual){
			return "L" + soluciona(arbol.hijoIz(), numerador, denominador);
		}else{
			return "";
		}
	}
