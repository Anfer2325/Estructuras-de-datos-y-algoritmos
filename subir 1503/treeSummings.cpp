...
bool treeSummings(Arbin<int> a, int suma, int num){
	if(a.esVacio()){
		return false;
	}else{
		suma=suma+a.raiz();
		if(suma==num && a.hijoIz().esVacio() && a.hijoDr().esVacio()){
			return true;
		}else if(suma>=num){
			return false;
		}else{
			return treeSummings(a.hijoIz(),suma,num) || treeSummings(a.hijoDr(),suma,num);
		}
	}
}

int main() {
	/*muestraIndentado(arbolSencillo(), 0);
	muestraConParentesis(arbolSencillo(), 0);
	muestraEnPreorden(arbolSencillo(), 0);*/
	
	/* 
	 * prueba a copiar la salida de 
	 *   muestraConParentesis() y
	 * 	 muestraEnPreorden() a un fichero, y
	 *   a leerla con el código siguiente:
	 */ 
	unsigned int num;
	cin>>num;
	do{
		Arbin<int> a = leeArbolConParentesis();
		//Arbin<int> a = leeArbolEnPreorden();
		if(treeSummings(a,0,num)){
			cout << "yes";
		}else{
			cout << "no";
		}
	...
	/*muestraConParentesis(leeArbolConParentesis(), 0);	
	muestraEnPreorden(leeArbolEnPreorden(), 0);	*/
	return 0;
}
