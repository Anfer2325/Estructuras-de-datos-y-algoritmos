...
void imprimeReceta(Arbus<int, string> &rCoste, Arbus<string, int> &rNombre){
	if(!rCoste.esVacio()){
		Arbus<int, string>::Iterador itc = rCoste.principio();
		while(itc!=rCoste.final()){
			int coste = itc.clave();

			Arbus<string, int>::Iterador itn = rNombre.principio();
			while(itn!=rNombre.final()){
				if(coste == itn.valor()){
					string nombre = itn.clave();
					cout << nombre << endl;
					itn.avanza();
					rNombre.borra(nombre); //Vamos borrando el arbol rNombre
				}else{
					itn.avanza();
				}
			}
			...
			itc.avanza();
			rCoste.borra(coste); //Vamos borrando el arbol rCoste
		}
...
int main(){
	int numCarpetas, ingredientes, recetas, presupuesto, costeIng, coste=0, n;
	string nombreCarpeta, nombreReceta, nombreIng;
	Arbus <string, int> ing;
	Arbus <int , string> recCoste;
	Arbus <string , int> recNombre; 

	cin>>numCarpetas;

	//cout << "Carpetas: " << numCarpetas << endl;
	while(numCarpetas!=0){
		ignoraEspacios(cin);
		getline(cin, nombreCarpeta);
		ignoraEspacios(cin);
		//cout << "Carpeta**: " << nombreCarpeta << endl;
		cin >> ingredientes;
		cin >> recetas;
		cin >> presupuesto;
		//cout << ingredientes << " " << recetas << " " << presupuesto << endl;
		while(ingredientes!=0){ //Leo todos los ingredientes y los meto en el arbol
			cin >> nombreIng;
			cin >> costeIng;
			ing.inserta(nombreIng, costeIng);
			ingredientes--;
		}
		while(recetas!=0){ // voy leyendo todas las recetas
			ignoraEspacios(cin);
			getline(cin, nombreReceta);
			ignoraEspacios(cin);
			cin >> ingredientes;

			while(ingredientes!=0){ //Leo los ingredientes y calculo el coste
				cin >> nombreIng;
				cin >> n;
				coste = coste + n*ing.consulta(nombreIng);
				ingredientes--;
			}
			if(coste <= presupuesto){
				recCoste.inserta(coste, nombreReceta);
				recNombre.inserta(nombreReceta, coste);
			}
			coste = 0;
			recetas--;
		}
		cout << nombreCarpeta << endl;
		imprimeReceta(recCoste, recNombre);
		numCarpetas--;
	}
	return 0;
}