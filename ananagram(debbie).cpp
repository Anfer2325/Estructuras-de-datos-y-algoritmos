...
void Ananagrams (){
	Arbus <string, int> arbContador;
	Arbus <string, string> arbol;
	Lista <string> l;
	char cad[20];
	string palabra = " ";
	string aux;
	...
	while ( !cin.eof() || palabra != "\n"){
		...
		if ( palabra !=  " " && palabra != "\n"){
			aux = palabra;
			for ( int unsigned i = 0; i < aux.length(); i++){
				aux[i] = tolower ( palabra[i]);
			}

			slot(aux.begin(), aux.end());

			if ( arbol.esta(aux)){
				arbContador.inserta(aux, arbContador.consulta(aux) + 1);
				cout << "insertado segundo" << endl;
			}
			else {
				arbol.inserta( aux, palabra);
				arbContador.inserta( aux, 1);
				cout << "insertado primero" << endl;
			}
		} else if ( palabra == "\n"){
		}
	}
