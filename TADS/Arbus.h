...
public:
	...
	/**
	Operacion del ejercicio 1 del parcial de junio de 2012
	Devuelve la siguiente clave de una clave k sin usar iteradores
	**/
	int siguienteClave(const Clave k){
		if(_ra == NULL){ //Si es nulo devuelvo k-1 (para saber que no hay siguiente por ahi)
			return k-1;
		}else if (_ra->_clave < k){ //si es menor busco por la rama derecha
			_ra = _ra->_dr;
			return siguienteClave(k); 
		}else if (_ra->_clave > k){ //si es mayor busco por la rama izq
			int miClave = _ra->_clave;
			_ra = _ra->_iz;
			int mejorIz = siguienteClave(k);
			if(mejorIz==k-1){
				return miClave;
			}else{
				return mejorIz;
			}
		}else{ //si es igual hago recursion por la izq y por la dcha
			Nodo *act = _ra;
			Nodo *hizq = _ra->_iz;
			Nodo *hdr = _ra->_dr;
			int miClave = _ra->_clave; //clave del nodo en el que estoy
			_ra = hizq;
			int mejorIzq = siguienteClave(k); //Mejor por la izquierda
			_ra = hdr;
			int mejorDr = siguienteClave(k); //Mejor por la derecha
			if(mejorIzq==k-1 && mejorDr==k-1){ //No hay ninguno mejor que el nodo en el que estoy
				return miClave;
			}else if(mejorDr==k-1){ //Es mejor el que he encontrado por la derecha
				return mejorIzq;
			}else if(mejorIzq==k-1){ //es mejor el que he encontrado por la izq
				return mejorDr;
			}else{
				if(mejorIzq<mejorDr){
					return mejorIzq;
